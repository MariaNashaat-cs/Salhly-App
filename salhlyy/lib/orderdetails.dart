...
class OrderDetailsLayout extends StatefulWidget {
  ...
  @override
  State<OrderDetailsLayout> createState() => _OrderDetailsLayoutState();
}

class _OrderDetailsLayoutState extends State<OrderDetailsLayout> with SingleTickerProviderStateMixin {
  String _fullName = '';
  String _address = '';
    ...
    try {
      ...
      if (widget.phoneNumber.isNotEmpty) {
        ...
        if (snapshot.exists) {
          ...
          if (userData['current_appointment'] != null) {
            final appointmentData = userData['current_appointment'] as Map<dynamic, dynamic>;
            setState(() {
  ...
  Future<void> _updateServiceRequestStatus(String orderId, String newStatus) async {
    try {
      ...
      debugPrint('Updating status for orderId: $orderId to $newStatus');
      await _database
          .child('serviceRequests')
          .child(orderId)
          .update({
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    // Navigate to RefundPage for card/visa payments
                                            ...
                                            Expanded(
                                              child: Padding(
                                                ...
                                                child: ElevatedButton.icon(
                                                  ...
                                                  label: const Text('Chat with Fixer'),
                                                  style: ElevatedButton.styleFrom(
                                                    backgroundColor: const Color(0xFF0C5FB3).withOpacity(0.8),
                                                    disabledBackgroundColor: Colors.grey[400],
                                                    foregroundColor: Colors.white,
...
class RequestInfoComponent extends StatelessWidget {
  ...
  const RequestInfoComponent({
    super.key,
    this.serviceType = "Plumbing Repair",
    this.status = "In Progress",
    required this.appointmentDate,
    required this.appointmentTime,
    this.paymentAmount = "150 L.E",
    required this.paymentMethod,
  });
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: const Icon(Icons.home_repair_service, color: Color(0xFF0C5FB3)),
              ),
              const SizedBox(width: 12),
              Text(
          ...
          const SizedBox(height: 20),
          _buildServiceRow('Service Type', serviceType, Icons.build),
  ...
  Widget _buildPaymentMethodRow(String value, IconData icon) {
    ...
    if (value.toLowerCase().contains('card') || value.toLowerCase().contains('visa') ||
        value.toLowerCase().contains('credit') || value.toLowerCase().contains('debit')) {
