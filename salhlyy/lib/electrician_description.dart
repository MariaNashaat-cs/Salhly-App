...
import 'electrician_time&date.dart';
...
class ElectricianDescription extends StatefulWidget {
  final Map<String, Map<String, int>> problemPriceRanges;
  final String phoneNumber;

  const ElectricianDescription({
    super.key,
  ...
  @override
  _ElectricianDescriptionState createState() => _ElectricianDescriptionState();
}

class _ElectricianDescriptionState extends State<ElectricianDescription> {
      ...
      if (snapshot.exists && snapshot.value != null) {
        if (snapshot.value is Map) {
          ...
          if (data['description'] != null && data['description'] is Map) {
            final descriptionData = (data['description'] as Map).cast<String, dynamic>();
            setState(() {
              _noteController.text = descriptionData['note']?.toString() ?? '';
              _selectedImages.clear();
              _selectedVideos.clear();
              _recordedAudios.clear();

              if (descriptionData['images'] != null && descriptionData['images'] is List) {
                final imagePaths = List<String>.from(descriptionData['images'].map((e) => e.toString()));
                _selectedImages.addAll(imagePaths.map((path) => File(path)).where((file) => file.existsSync()));
              }

              if (descriptionData['videos'] != null && descriptionData['videos'] is List) {
                final videoPaths = List<String>.from(descriptionData['videos'].map((e) => e.toString()));
              ...
              if (descriptionData['audios'] != null && descriptionData['audios'] is List) {
                final audios = List<dynamic>.from(descriptionData['audios']);
                for (var audio in audios) {
                  if (audio is Map) {
                    final audioData = audio.cast<String, dynamic>();
                    final filePath = audioData['path']?.toString();
                    final duration = audioData['duration'];
                    if (filePath != null && duration is int) {
                      final file = File(filePath);
                      if (file.existsSync()) {
                        _recordedAudios.add(AudioRecording(file: file, duration: duration));
                      }
                    }
                  }
                }
        ...
        print('No data exists at the specified path');
      }
    } catch (e) {
  ...
  Future<void> _saveData() async {
    try {
      Map<String, dynamic> sanitizedProblemsData = {};
      widget.problemPriceRanges.forEach((key, value) {
        String sanitizedKey = _sanitizeKey(key);
        sanitizedProblemsData[sanitizedKey] = {
          'min': value['min'],
          'max': value['max'],
      ...
      await FirebaseDatabase.instance
          ...
          .child(widget.phoneNumber)
          .update({
        'electricalproblem': sanitizedProblemsData,
        'description': {
          'note': _noteController.text,
          'images': _selectedImages.map((file) => file.path).toList(),
          'videos': _selectedVideos.map((file) => file.path).toList(),
  ...
  bool _validateForm() {
    if (!_isTyping) {
      setState(() {
        _errorMessage = "Please add at least one note, image, audio, or video description";
      });
      return false;
    }
    setState(() {
  ...
  void _navigateToNext() {
    if (_validateForm()) {
      ...
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => ElectricianDateTimePickerScreen(phoneNumber: widget.phoneNumber)),
      );
    }
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text(
          ...
          style: TextStyle(fontFamily: 'Playfair_Display', fontSize: 24, fontWeight: FontWeight.bold, color: Color(0xFFFFFFFF)),
        ),
        backgroundColor: const Color(0xFF0C5FB3),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Text(_selectedImages.isNotEmpty ? 'Add More Videos' : 'Add Videos', style: TextStyle(fontSize: 18, fontFamily: 'Open Sans', color: const Color(0xFF0C5FB3), fontWeight: FontWeight.w500)),
                const SizedBox(width: 10),
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const ElectricianDescription(
        problemPriceRanges: {},
