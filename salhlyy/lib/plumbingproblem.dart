...
import 'video.dart'; // Placeholder import (ensure this file exists)
import 'description.dart'; // Placeholder import (ensure this file exists)
import 'homepage.dart'; // Placeholder import (ensure this file exists)
...
// HeaderComponent with responsive constraints
  ...
  Widget build(BuildContext context) {
    return Container(
      constraints: BoxConstraints(
        maxWidth: MediaQuery.of(context).size.width * 0.9, // Responsive maxWidth
      ),
      height: 45,
...
class ProblemListComponent extends StatefulWidget {
  ...
  const ProblemListComponent({
    super.key,
    this.problems = const [
      {'name': 'Filter', 'icon': Icons.filter_alt},
      {'name': 'Toilet', 'icon': Icons.wc},
      {'name': 'Sink', 'icon': Icons.wash},
      {'name': 'Bath', 'icon': Icons.bathtub},
      {'name': 'Flush', 'icon': Icons.water_damage},
      {'name': 'Taps and faucets', 'icon': Icons.water},
      {'name': 'Shower', 'icon': Icons.shower},
      {'name': 'Water heater', 'icon': Icons.hot_tub},
...
class _ProblemListComponentState extends State<ProblemListComponent> {
  ...
  Future<void> _saveProblems() async {
    try {
      await FirebaseDatabase.instance
          ...
          .update({
        'plumbingproblem': problemPriceRanges.map((key, value) => MapEntry(key, value))
      });
    } catch (e) {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: const Text(
              'You need an account to access this feature. Please sign up or log in to continue.'),
          ...
          actions: [
            TextButton(
              ...
              child: const Text('OK',
                  style: TextStyle(fontSize: 16, color: Color(0xFF0C5FB3))),
            ),
          ],
        );
      },
  ...
  Widget build(BuildContext context) {
    return Container(
      constraints: BoxConstraints(
        maxWidth: MediaQuery.of(context).size.width * 0.85, // Responsive maxWidth
      ),
      ...
      child: Column(
        ...
        children: widget.problems
            .map((problem) =>
                _buildProblemItem(problem['name'], problem['icon'], context))
            .toList(),
      ),
    );
  }
          ...
          onTap: () {
            if (widget.isGuest) {
              ...
              if (selectedProblems.contains(problem)) {
                ...
                switch (problem) {
                  case 'Filter':
                    problemPriceRanges[problem] = {'min': 200, 'max': 500};
                    break;
                  case 'Toilet':
                    problemPriceRanges[problem] = {'min': 150, 'max': 1500};
                    break;
                  case 'Sink':
                    problemPriceRanges[problem] = {'min': 200, 'max': 1200};
                    break;
                  case 'Bath':
                    problemPriceRanges[problem] = {'min': 300, 'max': 3000};
                    break;
                  case 'Flush':
                    problemPriceRanges[problem] = {'min': 200, 'max': 200};
                    break;
                  case 'Taps and faucets':
                    problemPriceRanges[problem] = {'min': 300, 'max': 300};
                    break;
                  case 'Shower':
                    problemPriceRanges[problem] = {'min': 200, 'max': 2000};
                    break;
                  case 'Water heater':
                    problemPriceRanges[problem] = {'min': 500, 'max': 5000};
                    break;
                  case 'Others':
                    problemPriceRanges[problem] = {'min': 80, 'max': 120};
                    break;
                  default:
                    problemPriceRanges[problem] = {'min': 100, 'max': 200};
                }

                if (problem == 'Filter' || problem == 'Water heater') {
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Row(
            children: [
              Icon(
                problem == 'Filter' ? Icons.filter_alt : Icons.hot_tub,
            ...
            ElevatedButton(
              onPressed: () {
                ...
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) =>
                        VideoScreen(username: widget.phoneNumber),
                  ),
                );
              },
...
class PlumbingProblemLayout extends StatefulWidget {
  final String username;
  const PlumbingProblemLayout({super.key, required this.username});

  String get phoneNumber => username;

  @override
  _PlumbingProblemLayoutState createState() => _PlumbingProblemLayoutState();
}

class _PlumbingProblemLayoutState extends State<PlumbingProblemLayout> {
  Set<String> selectedProblems = {};
  Map<String, Map<String, int>> problemPriceRanges = {};

  void _updateSelectedProblems(
      Set<String> problems, Map<String, Map<String, int>> priceRanges) {
    setState(() {
            ...
            onPressed: () {
              showDialog(
                ...
                builder: (context) => AlertDialog(
                  ...
                  content: const Text(
                      'Select the areas that need plumbing service. You can select multiple options.'),
                  actions: [
                    TextButton(
                                  ...
                                  showDialog(
                                    ...
                                    builder: (context) {
                                      return AlertDialog(
                                        title: const Text('Account Required'),
                                        content: const Text(
                                            'You need an account to proceed further. Please sign up or log in to continue.'),
                                        shape: RoundedRectangleBorder(
                                            borderRadius:
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const PlumbingProblemLayout(username: "exampleUser"),
    );
  }
}