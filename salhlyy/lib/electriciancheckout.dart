...
import 'electricianorderdetails.dart';
import 'firebase_options.dart';
import 'dart:math';

// Define app theme constants for consistency
class AppTheme {
...
class Electriciancheckout extends StatefulWidget {
  ...
  @override
  State<Electriciancheckout> createState() => _CheckoutLayoutState();
}

class _CheckoutLayoutState extends State<Electriciancheckout> {
  bool _isProcessingPayment = false;
  final DatabaseReference _database = FirebaseDatabase.instance.ref();
  final ValueNotifier<double> _totalAmountNotifier = ValueNotifier<double>(0.0);
  double _discountAmount = 0.0;
  Map<String, dynamic> electricalProblems = {};
  ...
  Future<void> _loadElectricalProblems() async {
    try {
      ...
      if (snapshot.exists) {
        ...
        setState(() {
          electricalProblems = formattedProblems;
          _isLoading = false;
        });
      } else {
        setState(() {
      ...
      debugPrint('Error loading electrical problems: $e');
      setState(() {
        _isLoading = false;
      });
    }
  ...
  Future<void> _saveOrderToFirebase(String paymentMethod, double totalAmount) async {
    try {
      ...
      if (_useWalletBalance && _currentWalletBalance > 0) {
        final totalBeforeBalance = widget.fixerPrice + widget.serviceFee + widget.taxAmount - _discountAmount;
      ...
      if (mounted) {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => Electricianorderdetails(
              username: _userName,
              paymentMethod: paymentMethod,
...
class _EnhancedCreditCardFormState extends State<EnhancedCreditCardForm> {
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      child: _cardAdded ? _buildCardDetailsBox(context) : _buildCreditCardForm(context),
    );
  }
  ...
  Widget _buildCardDetailsBox(BuildContext context) {
    return GestureDetector(
      onTap: () {
        setState(() {
          _cardAdded = false;
        ...
        children: [
          ...
          Builder(
            builder: (context) {
              return Column(
                children: [
                  for (var entry in electricalProblems.entries)
